function Cloud (x, y, width, height, imageData, killable, movementFunction = null) {
	this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
	...
	this.draw = function (context, camera) {
        if (this.imageData.spriteSheet === true) {
            ...
            context.drawImage(this.imageData.image, this.currentFrame * imageWidth, 0, imageWidth, this.imageData.image.height, this.x + ((this.movementFuntion == null) ? 0: this.movementFuntion(this.t)) + ((this.position === "absolute") ? 0: (camera.xOffset - ((camera.trackedObject == sprite) ? sprite.xVelocity: 0))), this.y + (camera.yOffset - ((camera.trackedObject == sprite) ? sprite.yVelocity: 0)), this.width, this.height);
        } else context.drawImage(this.imageData.image, 0, 0, this.imageData.image.width, this.imageData.image.height, this.x + ((this.movementFuntion == null) ? 0: this.movementFuntion(this.t)) + ((this.position === "absolute") ? 0: (camera.xOffset - ((camera.trackedObject == sprite) ? sprite.xVelocity: 0))), this.y + (camera.yOffset - ((camera.trackedObject == sprite) ? sprite.yVelocity: 0)), this.width, this.height);
    }
    ...
    this.update = function (context, camera) {
		...
		//for (let i = 0; i < 5; i++) if (Math.floor(Math.random() * 5) == 0) particles.push(new Particle((this.x + this.width / 2) + (Math.random() * this.width - this.width / 2), this.y + this.height / 2, 0, (this.snowy == 1) ? 5: 0, 10, (this.snowy == 1) ? false: true, (this.snowy == 1) ? "#FFFFFF": "#0077FF"));
		this.x += 0;
